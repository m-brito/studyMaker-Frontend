...
async function mostrarQuestionarios() {
    ...
    const meusQuestionarios = await buscarMeusQuestionarios();
...
// APIs
async function buscarMeusQuestionarios() {
    var tentativas = 0;
    var ok = false
    while(tentativas <= 4 && ok == false) {
        try {
            const resp = await fetch(`${HOST}/questionario/dadosMeusQuestionarioMateria.php`, {
                ...
                body: JSON.stringify({
                    "token": buscarToken(),
                    "idMateria": parametrosJsonQuestionarios["idMateria"],
                    "idCurso": parametrosJsonQuestionarios["idCurso"],
                })
            })
