...
async function iniciarRevisaoQuestionarios(parametrosJson) {
    ...
    if(respHistorico["status"] != erroRequisicao) {
        const respBuscarQuestionario = await buscarQuestionarioCompletoAPI(parametrosJsonRevisaoQuestionario["idQuestionario"]);
        if(respBuscarQuestionario["status"] == erroRequisicao || respBuscarQuestionario["resultados"][0]["idMateria"] != parametrosJsonRevisaoQuestionario["idMateria"]) {
            mensagemPopUp.show({
                mensagem: "Tivemos problemas ao acessar esse questionario!",
                cor: "red"
            });
            voltarPagina();
        } else if(respHistorico["resultados"]["historico"]["idQuestionario"] == respBuscarQuestionario["resultados"][0]["id"]) {
            ...
            mensagemPopUp.show({
                mensagem: "Não foi possivel encontrar esse historico.<br>Talvez o historico não seja dessa questionario",
                cor: "orange"
            });
            voltarPagina();
        }
        ...
        mensagemPopUp.show({
            mensagem: respHistorico["mensagem"],
            cor: "red"
        });
        voltarPagina();
    }
